...
namespace GMEPPlumbing.Tools
{
  ...
  {
    public List<WasteEntry> Entries { get; set; } =
      new List<WasteEntry>()
      {
    ...
    public string FindSize(double dfu, string type, double length = 0)
    {
      ...
      {
        entry = Entries
          .Where(e => e.OnePercentSlopeDfu >= dfu)
          .OrderBy(e => e.OnePercentSlopeDfu)
          .FirstOrDefault();
      }
      else if (type == "2%")
      {
        entry = Entries
          .Where(e => e.TwoPercentSlopeDfu >= dfu)
          .OrderBy(e => e.TwoPercentSlopeDfu)
          .FirstOrDefault();
      }
      else if (type == "Vertical")
      {
        entry = Entries
          .Where(e => e.VerticalDfu >= dfu && e.MaxVerticalLength >= length)
          .OrderBy(e => e.VerticalDfu)
          .FirstOrDefault();
      }
  ...
  {
    public List<VentEntry> Entries { get; set; } =
      new List<VentEntry>()
      {
    ...
    {
      string size = "Pipe Size: N/A";
      VentEntry entry = Entries
        .Where(e => e.MaxDFU >= dfu && e.MaxLength >= length)
        .OrderBy(e => e.MaxDFU)
        .FirstOrDefault();
      if (entry == null)
        return size;
  ...
  class WasteEntry
  {
    ...
    public WasteEntry(
      double verticalDfu,
      double twoPercentSlopeDfu,
      double onePercentSlopeDfu,
      double maxVerticalLength,
      string pipeDiameter
    )
    {
