...
namespace GMEPPlumbing
{
  public class AutoCADIntegration
  {
    ...
    public void PlumbingFixture()
    {
      ...
      if (!String.IsNullOrEmpty(selectedFixtureType.WaterGasBlockName))
      {
        ...
        foreach (string waterGasBlockName in waterGasBlockNames)
        {
          ...
          try
          {
            CreateColdWaterHotWaterDownBlock(
              projectId,
              selectedCatalogItem,
              selectedFixtureType,
              index,
    ...
    public void CreateColdWaterHotWaterDownBlock(
      string projectId,
      PlumbingFixtureCatalogItem selectedCatalogItem,
      PlumbingFixtureType selectedFixtureType,
      int index,
      string blockName,
      string basePointId
    )
    {
      ...
      MakePlumbingFixtureWaterGasLabel(plumbingFixture, blockName, index);
    }
    ...
    public static List<PlumbingSource> GetPlumbingSourcesFromCAD(string ProjectId)
    {
      ...
      using (Transaction tr = db.TransactionManager.StartTransaction())
      {
        ...
        BlockTableRecord modelSpace = (BlockTableRecord)
          tr.GetObject(bt[BlockTableRecord.ModelSpace], OpenMode.ForRead);
        List<string> blockNames = new List<string> { "GMEP SOURCE", "GMEP WH 80", "GMEP WH 50" };
