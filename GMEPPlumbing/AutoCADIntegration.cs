    ...
    public class AutoCADIntegration
    {
        ...
        public async void PlumbingVerticalRoute()
        {
                    ...
                    if (id.IsValid)
                    {
                        ...
                        )
                        {
                            if (anonymousBtr != null)
                            {
                                ...
                                {
                                    var entity =
                                        tr.GetObject(objId, OpenMode.ForRead) as BlockReference;
            ...
            using (Transaction tr = db.TransactionManager.StartTransaction())
            {
                ...
                BlockTableRecord basePointBlock = (BlockTableRecord)
                    tr.GetObject(bt["GMEP_PLUMBING_BASEPOINT"], OpenMode.ForRead);
                Dictionary<string, List<ObjectId>> basePoints =
            ...
            if (endFloor > startFloor)
            {
                ...
                using (Transaction tr = db.TransactionManager.StartTransaction())
                {
                    ...
                    BlockTable bt = (BlockTable)tr.GetObject(db.BlockTableId, OpenMode.ForRead);
                    BlockReference startPipe =
                        tr.GetObject(startPipeId, OpenMode.ForWrite) as BlockReference;
                    startPipe.Erase(true);

                    //start pipe
                    Point3d newUpPointLocation2 = BasePointRefs[startFloor].Position + upVector;
                    BlockTableRecord blockDef2 =
                        tr.GetObject(bt["GMEP_PLUMBING_LINE_UP"], OpenMode.ForRead)
                        as BlockTableRecord;
                    BlockTableRecord curSpace2 = (BlockTableRecord)
                        tr.GetObject(db.CurrentSpaceId, OpenMode.ForWrite);
                    BlockReference upBlockRef2 = new BlockReference(
                        newUpPointLocation2,
                        blockDef2.ObjectId
                    );
                ...
                using (Transaction tr = db.TransactionManager.StartTransaction())
                {
                    ...
                    for (int i = startFloor + 1; i < endFloor; i++)
                    {
                        ...
                        BlockTableRecord blockDef =
                            tr.GetObject(bt["GMEP_PLUMBING_LINE_VERTICAL"], OpenMode.ForRead)
                            as BlockTableRecord;
                        BlockTableRecord curSpace = (BlockTableRecord)
                            tr.GetObject(db.CurrentSpaceId, OpenMode.ForWrite);

                        // Create the BlockReference at the desired location
                        BlockReference upBlockRef = new BlockReference(
                            newUpPointLocation,
                            blockDef.ObjectId
                        );
                    ...
                    BlockTableRecord blockDef3 =
                        tr.GetObject(bt["GMEP_PLUMBING_LINE_DOWN"], OpenMode.ForRead)
                        as BlockTableRecord;
                    BlockTableRecord curSpace3 = (BlockTableRecord)
                        tr.GetObject(db.CurrentSpaceId, OpenMode.ForWrite);
                    BlockReference upBlockRef3 = new BlockReference(
                        newUpPointLocation3,
                        blockDef3.ObjectId
                    );
        ...
        private void AttachRouteXData(ObjectId lineId, string id, string basePointId)
        {
            ...
            using (Transaction tr = db.TransactionManager.StartTransaction())
            {
                ...
                RegAppTable regAppTable = (RegAppTable)
        ...
        private void UpdateXRecordId(Transaction tr, string newId, DateTime newCreationTime)
        {
            ...
            {
                Xrecord xRec = (Xrecord)
                    tr.GetObject(namedObjDict.GetAt(XRecordKey), OpenMode.ForWrite);
        ...
        private void MakePlumbingFixtureWaterGasLabel(
            PlumbingFixture fixture,
            PlumbingFixtureType type
        )
        {
            double distance = 3;
        ...
        public void PlumbingFixture()
        {
            ...
            {
                selectedFixtureType = plumbingFixtureTypes.FirstOrDefault(t =>
                    t.Abbreviation == "WC"
                );
            }
            ...
            PlumbingFixtureCatalogItem selectedCatalogItem =
                plumbingFixtureCatalogItems.FirstOrDefault(i =>
                    i.Id.ToString() == keywordResultString
                );
            ...
            if (selectedFixtureType.WaterGasBlockName.Contains("%WHSIZE%"))
            {
                if (selectedFixtureType.Abbreviation == "WH")
                {
                    ...
                    selectedFixtureType.WaterGasBlockName =
                        selectedFixtureType.WaterGasBlockName.Replace("%WHSIZE%", whSize);
                }
            }
            ...
            if (selectedFixtureType.WasteVentBlockName.Contains("%FSSIZE%"))
            {
                if (selectedFixtureType.Abbreviation == "FS")
                {
                    ...
                    selectedFixtureType.WasteVentBlockName =
                        selectedFixtureType.WasteVentBlockName.Replace("%FSSIZE%", fsSize);
                }
            }

            if (!String.IsNullOrEmpty(selectedFixtureType.WaterGasBlockName))
            {
                string[] waterGasBlockNames = selectedFixtureType.WaterGasBlockName.Split(',');
                int index = 0;
                Point3d downPosition = new Point3d();
                double downRotation = 0;
                ObjectId blockId;
                Point3d point;

                foreach (string waterGasBlockName in waterGasBlockNames)
                {
                    ed.WriteMessage("\nSelect base point for " + selectedFixtureType.Name);
                    double rotation = 0;
                    string fixtureId = Guid.NewGuid().ToString();
                    string blockName = waterGasBlockName;
                    try
                    {
                        if (waterGasBlockName == "GMEP CW HW DN")
                        {
                            // (downPosition, downRotation) = CreateColdWaterHotWaterDownBlock();
                        }
                        using (Transaction tr = db.TransactionManager.StartTransaction())
                        {
                            BlockTable bt = (BlockTable)
                        ...
                        using (Transaction tr = db.TransactionManager.StartTransaction())
                        {
                            ...
                            BlockReference br = (BlockReference)
                                tr.GetObject(blockId, OpenMode.ForWrite);
        ...
        public void CreatePlumbingSource()
        {
            ...
            {
                selectedSourceType = plumbingSourceTypes.FirstOrDefault(t =>
                    t.Type == "Water Meter"
                );
            }
        ...
        )
        {
            ...
            try
            {
                ...
                using (Transaction tr = db.TransactionManager.StartTransaction())
                {
                    ...
                    //MariaDBService.CreatePlumbingFixture(plumbingFixture);
                    MakePlumbingFixtureWasteVentLabel(
                        plumbingFixture,
                        br.Position,
                        blockName,
                        index
                                ...
                                foreach (ObjectId id in basePointBlock.GetAnonymousBlockIds())
                                {
                                    if (id.IsValid)
                                    {
                                        ...
                                        )
                                        {
                                            ...
                                            {
                                                foreach (
                                                    ObjectId objId in anonymousBtr.GetBlockReferenceIds(
                                                        true,
                                                        false
                                                    )
                                                )
                                                {
                                                    if (objId.IsValid)
                                                    {
                                                        ...
                                                        var pc =
                                                            entity.DynamicBlockReferencePropertyCollection;

                                                        foreach (
                                                            DynamicBlockReferenceProperty prop in pc
                                                        )
                                                        {
                                                            if (
                                                                prop.PropertyName
                                    ...
                                    )
                                    {
                                        ...
                                        foreach (DynamicBlockReferenceProperty prop in pc)
                                        {
                                            if (prop.PropertyName == "Id")
                                            {
                                                ...
                                                if (
                                                    !string.IsNullOrEmpty(basePointId)
                                                    && basePointId != "0"
                                                )
                                                {
                                                ...
                                                if (objId.IsValid)
                                                {
                                                    ...
                                                    if (match)
                                                    {
                                                        BlockReference basePointRef2 =
                                                            tr.GetObject(
                                                                basePoints[BasePointId2],
                                                                OpenMode.ForRead
                                                            ) as BlockReference;
                                                        entity.Position =
                                                            basePointRef2.Position + distanceVector;
                                                    }
                                                }
                                            }
                                        }
                            ...
                            if (anonymousBtr != null)
                            {
                                ...
                                )
                                {
                                    if (objId.IsValid)
                                    {
                                        ...
                                        if (Id != "0")
                                        {
                                            PlumbingPlanBasePoint BasePoint =
                                                new PlumbingPlanBasePoint(
                                                    Id,
                                                    ProjectId,
                                                    entity.Position,
                                                    Plan,
