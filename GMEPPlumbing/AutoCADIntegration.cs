...
namespace GMEPPlumbing
{
  public class AutoCADIntegration {
    ...
    private static readonly List<Tuple<string, string>> pendingDuplicationRoutes = new List<Tuple<string, string>>();
    ...
    public static async void Db_VerticalRouteModified(object sender, ObjectEventArgs e) {
      var doc = Application.DocumentManager.MdiActiveDocument;
      if (doc == null) return;
  ...
  public static List<PlumbingFixture> GetPlumbingFixturesFromCAD(string ProjectId = "") {
    ...
    public static void StageDuplicateFullVerticalRoute(PlumbingVerticalRoute route, ObjectId objid) {
      ...
      pendingDuplicationRoutes.Add(new Tuple<string, string>(id, oldVerticalRouteId));
    ...
    public static void DuplicateFullVerticalRoutes() {
      ...
      if (!SettingObjects && !IsSaving && pendingDuplicationRoutes.Count > 0) 
      {
        ...
        foreach (Tuple<string, string> p in routeKeys) {
          PlumbingVerticalRoute mainRoute = verticalRoutes.FirstOrDefault(r => r.Id == p.Item1);
          if (mainRoute == null) continue;
          PlumbingPlanBasePoint mainBasePoint = basePoints.FirstOrDefault(bp => bp.Id == mainRoute.BasePointId);
          if (mainBasePoint == null) continue;

          Vector3d basePointOffset = mainBasePoint.Point - mainRoute.Position;

          List<PlumbingVerticalRoute> routesToProcess = verticalRoutes.Where(r => r.VerticalRouteId == p.Item2).ToList();
          ...
          foreach (PlumbingVerticalRoute subRoute in routesToProcess) {
            ...
            using (Transaction tr = db.TransactionManager.StartTransaction()) {
              ...
              Point3d adjustedPosition = subRoute.Position + basePointOffset;
              BlockReference newBlockRef = new BlockReference(adjustedPosition, blockDef.ObjectId);
              ...
              foreach (DynamicBlockReferenceProperty prop in pc) {
                ...
                if (prop.PropertyName == "vertical_route_id") {
                  prop.Value = mainRoute.VerticalRouteId;
                }
                if (prop.PropertyName == "base_point_id") {
              ...
              pendingDuplicationRoutes.Remove(p);
              tr.Commit();
            }
          }
        }
