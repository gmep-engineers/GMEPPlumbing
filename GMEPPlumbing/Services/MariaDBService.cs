    ...
    public async Task InsertPlumbingRouteInfoBoxes(List<RouteInfoBox> boxes, string viewportId) {
      ...
      if (boxes.Count > 0) {
        string upsertQuery = @"
              ...
              (viewport_id, component_id, project_id, pipe_size, type, location_description, cfh, longest_run_length, direction_description, is_vertical_route, segment_length, source_description)
              VALUES (@viewportId, @componentId, @projectId, @pipeSize, @type, @locationDescription, @cfh, @longestRunLength, @directionDescription, @isVerticalRoute, @segmentLength, @sourceDescription)";
